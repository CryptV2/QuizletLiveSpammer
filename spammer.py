...
if(testCode(code)):
		...
		sys.exit()
BotName = input('Name of Bots: ') + '-'
BotAmount = input('Amount of Bots: ')
for _ in range(int(BotAmount)):
	token=getToken()
	sid=getSession(code,token)
	name=str(BotName)+str(random.randint(0,999999))
	asyncio.get_event_loop().run_until_complete(join(code,token,sid,name))
	print(colors.OKGREEN,"-Flooding Game as:",name,colors.ENDC)
